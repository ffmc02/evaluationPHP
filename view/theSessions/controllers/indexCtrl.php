...
if (isset($_POST['registration'])) {
    ...
    if (!empty($_POST['surname'])) {
        if (preg_match($regexTitle, $_POST['surname'])) {
            ...
            $formError['surname'] ='Veuiller ne mettre que des caractéres alphabétique!!!!!!!!!!';
        }
    } else {
        $formError['surname'] = 'Merci de remplir Votre Nom de Familles!!!';
    }
    if (!empty($_POST['firstname'])) {
        if(preg_match($regexTitle,$_POST['firstname'])){
        $user->firstname = htmlspecialchars($_POST['firstname']);
        } else {
            $formError['firstname'] ='Veuiller ne mettre que des caractéres alphabétique!!!!!!!!!!';
        }
    } else {
...
//          je verifie que le password entré par l'utilisateur et le meme que celui renseigner dans la base de donnée
            ...
            if ($validPassword) {
                ...
                $_SESSION['connect'] = 'OK';
                header("Location: views/vue2.php");
            }
        } else {
